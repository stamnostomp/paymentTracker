...
var allCols = require('color-name');
var CssWhat = require('css-what');
var deepEqual = require('deep-equal');
var tailwindcss = require('tailwindcss');
var resolveConfig = require('tailwindcss/resolveConfig.js');
...
var postcss__namespace = /*#__PURE__*/_interopNamespace(postcss);
var allCols__default = /*#__PURE__*/_interopDefaultLegacy(allCols);
var CssWhat__namespace = /*#__PURE__*/_interopNamespace(CssWhat);
...
const namedColors = Object.entries(allCols__default['default'])
...
function groupDeclarationBlocksByClass(postCssRoot, resolvedColors, logFunction) {
    ...
    function handleRule(mediaQuery, rule) {
        ...
        let selectors;
        try {
            selectors = CssWhat__namespace.parse(rule.selector);
        }
        catch (e) {
...
function addToSelectorList(propertiesBySelector, subselectors, properties) {
    ...
    const index = result.findIndex(elem => deepEqual__default['default'](elem.subselectors, subselectors));
    if (index >= 0) {
        result[index] = {
...
function recognizeSelectorRest(selector) {
    ...
    const rest = CssWhat__namespace.stringify([selector.slice(1)]);
...
async function run({ directory = "./src", moduleName = "Tailwind", postcssPlugins = [], tailwindConfig = defaultTailwindConfig, generateDocumentation = false, logFunction = console.log, }) {
    ...
    const postcssResult = await postcss__namespace.default([
        tailwindcss__default['default'](tailwindConfig_),
        ...postcssPlugins,
        afterTailwindPlugin
    ]).process(css, { from, to });
...
function asPostcssPlugin({ moduleName, tailwindConfig, generateDocumentation, logFunction, modulesGeneratedHook }) {
    ...
    return {
        ...
        async OnceExit(root) {
            ...
            const resolvedConfig = resolveConfig__default['default'](tailwindConfig);
...
async function writeGeneratedFiles({ directory, moduleName, logFunction, generated }) {
    const modulePath = path__default['default'].join.apply(null, moduleName.split("."));
    logFunction([
        "Saved",
        " - " + chalk__default['default'].blue(await writeFile(path__default['default'].resolve(directory, `${modulePath}/Utilities.elm`), generated.utilitiesModule)),
        " - " + chalk__default['default'].blue(await writeFile(path__default['default'].resolve(directory, `${modulePath}/Theme.elm`), generated.themeModule)),
        " - " + chalk__default['default'].blue(await writeFile(path__default['default'].resolve(directory, `${modulePath}/Breakpoints.elm`), generated.breakpointsModule)),
    ].join("\n"));
}
/*
 * Helpers
...
function warningsTailwindConfig(tailwindConfig, logFunction) {
    if (!(lodash.isEmpty(tailwindConfig.variants) && lodash.isArray(tailwindConfig.variants))) {
        logFunction(`${chalk__default['default'].bold.yellow("Warning:")} It is recommended to ${chalk__default['default'].bold(`set ${chalk__default['default'].green("`variants: []`")} in the tailwindConfig`)}, otherwise the generated files are huge.
Instead, you should use elm-css functions like ${chalk__default['default'].blue("Css.focus")} and ${chalk__default['default'].blue("Css.hover")}.
...
async function writeFile(fname, content) {
    const folder = path__default['default'].dirname(fname);
    await fs.promises.mkdir(folder, { recursive: true });
    await fs.promises.writeFile(fname, content);
    return path__default['default'].relative(".", fname);
}

exports.asPostcssPlugin = asPostcssPlugin;
exports.defaultTailwindConfig = defaultTailwindConfig;
exports.docs = docs;
exports.run = run;
