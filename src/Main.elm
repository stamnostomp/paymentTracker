...
view model =
    case model.viewPage of
        MainPage ->
            div [ class "max-w-lg mx-auto p-4 bg-everblush-bg text-everblush-text" ]
                [ h1 [ class "text-2xl font-bold mb-4 text-everblush-primary" ] [ text ("Payments - Total: $" ++ String.fromFloat (total model)) ]
                , div [ class "text-xl mb-4 text-everblush-text" ] [ text ("Total Payments This Month: $" ++ String.fromFloat (totalPaymentsThisMonth model)) ]
                , filterButtons model
                , button [ onClick TogglePaymentInputs, class "bg-everblush-primary text-everblush-white px-4 py-2 rounded mt-4" ] [ text "Add Payment" ]
                , viewPaymentInputs model
                , button [ onClick ToggleExpenseInputs, class "bg-everblush-red text-everblush-white px-4 py-2 rounded mt-4" ] [ text "Add Expense" ]
                , viewExpenseInputs model
                , button [ onClick (NavigateTo StatsPage), class "bg-everblush-accent text-everblush-white px-4 py-2 rounded mt-4" ] [ text "View Stats" ]
...
filterButtons model =
    div [ class "mb-4" ]
        [ select [ onInput SetViewMode, class "border border-everblush-white p-2 rounded bg-everblush-bg text-everblush-text" ]
    ...
    if model.showPaymentInputs then
        div [ class "my-4" ]
            [ input
                ...
                , class "border border-everblush-white p-2 rounded w-full bg-everblush-bg text-everblush-text"
                ] []
            , button [ onClick AddPayment, class "bg-everblush-primary text-everblush-white px-4 py-2 rounded mt-2" ] [ text "Submit Payment" ]
            ]
    else
        text ""
    ...
    if model.showExpenseInputs then
        div [ class "my-4" ]
            [ input
                ...
                , class "border border-everblush-white p-2 rounded w-full bg-everblush-bg text-everblush-text mb-2"
                ] []
            , input
            ...
            , button [ onClick AddExpense, class "bg-everblush-red text-everblush-white px-4 py-2 rounded mt-2" ] [ text "Submit Expense" ]
            ]
    else
        text ""
...
viewPayment model payment =
    let
        isEditing = case model.editMode of
            ...
            _ -> False
    in
    li [ class "border-b py-2 text-everblush-text" ]
        [ if isEditing then
            div []
                [ input
                ...
                , button [ onClick (SaveEdit payment.id), class "bg-everblush-primary text-everblush-white px-4 py-2 rounded mt-2" ] [ text "Save" ]
                , button [ onClick (CancelEdit payment.id), class "bg-everblush-accent text-everblush-white px-4 py-2 rounded mt-2 ml-2" ] [ text "Cancel" ]
                , button [ onClick (DeleteEntry (payment.id, "payment")), class "bg-everblush-red text-everblush-white px-4 py-2 rounded mt-2 ml-2" ] [ text "Delete" ]
                ]
          else
            div [ class "flex justify-between" ]
                [ text (payment.description ++ ": $" ++ String.fromFloat payment.amount ++ " on " ++ payment.date)
                , button [ onClick (EditEntry (payment.id, "payment")), class "bg-everblush-primary text-everblush-white px-4 py-2 rounded" ] [ text "Edit" ]
                ]
        ]
    ...
    li [ class "border-b py-2 text-everblush-text" ]
        [ if isEditing then
            div []
                ...
                , button [ onClick (SaveEdit expense.id), class "bg-everblush-primary text-everblush-white px-4 py-2 rounded mt-2" ] [ text "Save" ]
                , button [ onClick (CancelEdit expense.id), class "bg-everblush-accent text-everblush-white px-4 py-2 rounded mt-2 ml-2" ] [ text "Cancel" ]
                , button [ onClick (DeleteEntry (expense.id, "expense")), class "bg-everblush-red text-everblush-white px-4 py-2 rounded mt-2 ml-2" ] [ text "Delete" ]
                ]
          else
            div [ class "flex justify-between" ]
                [ text (expense.description ++ ": $" ++ String.fromFloat expense.amount ++ " on " ++ expense.date)
                , button [ onClick (EditEntry (expense.id, "expense")), class "bg-everblush-primary text-everblush-white px-4 py-2 rounded" ] [ text "Edit" ]
                ]
        ]
...
viewStats model =
    let
        totalPayments = total model
        averageWeeklyPayment = totalPayments / 4
        target = 500.0
    in
    div [ class "max-w-lg mx-auto p-4 bg-everblush-bg text-everblush-text" ]
        [ h1 [ class "text-2xl font-bold mb-4 text-everblush-primary" ] [ text "Stats" ]
        , div [ class "text-xl mb-4 text-everblush-text" ] [ text ("Total Payments: $" ++ String.fromFloat totalPayments) ]
        , div [ class "text-xl mb-4 text-everblush-text" ] [ text ("Average Weekly Payment: $" ++ String.fromFloat averageWeeklyPayment) ]
        , div [ class "text-xl mb-4 text-everblush-text" ] [ text ("Target: $" ++ String.fromFloat target) ]
        , button [ onClick (NavigateTo MainPage), class "bg-everblush-primary text-everblush-white px-4 py-2 rounded mt-4" ] [ text "Back to Main" ]
        ]
